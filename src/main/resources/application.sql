select distinct on (la.input_application_number) la.input_application_number                  AS "Уникальный номер заявления",
                                                 ...
                                                 (SELECT CASE
                                                             WHEN sub.code is null THEN la.rf_subject_id::varchar
                                                             ELSE sub.code END)               AS "Код субъекта РФ",
                                                 (SELECT CASE
                                                             WHEN sub.name is null THEN
                                                                 (select nsisub.name
                                                                  from nsi.nsi_rf_subjects_codes nsisub
                                                                  where la.rf_subject_id = nsisub.id)
                                                             ELSE sub.name END)               AS "Субъект РФ",
                                                 appsubmit.name                               AS "Способ направления",
                                                 contype.name                                 AS "Тип заявителя",
                                                 (select case
                                                             when applicant.full_name is null then applicant.short_name
                                                             else applicant.full_name end)    AS "Наименование заявителя (для ЮЛ), ФИО для ФЛ и ИП",
                                                 applicant.ogrn                               AS "ОГРН (ОГРНИП) заявителя (для ЮЛ и ИП)",
                                                 applicant.inn                                AS "ИНН заявителя",
                                                 (select decision.name
                                                  from nsi.nsi_decision_result decision
                                                  where dec.decision_result_id = decision.id) AS "Решение",
